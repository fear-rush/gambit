...
export default class BITGET extends Exchange {
  ...
  formatRemoteToLocalPair(pair: string, type: string): string {
    return type === 'spot'
      ? `${pair}-SPOT`
      : type === 'coin-futures'
        ? pair.replace(/_CM$/, '')
  ...
  formatLocalToRemotePair(pair: string, type: string): string {
    return type === 'spot'
  ...
  hasConnectedFuturesPairs(api: Api): boolean {
  ...
  formatTrade(trade, pair: string, instType: string) {
  ...
  onMessage(event: MessageEvent, api: Api) {
    ...
    if (json.arg.topic === 'liquidation') {
      ...
      if (liquidations.length) {
        return this.emitLiquidations(api._id, liquidations)
      }
    }
  }
